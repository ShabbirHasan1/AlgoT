...
from data_engine import SymphonyFintechAPI, SQLDatabase
import constants
...
class SpotCandleEngine:
    ...
    async def fetch_quotes(self, instruments, message_code):
        try:
            while True:
                response = self.symphony_api.get_quote(instruments, message_code=message_code)
                self.formatted_quote = self.symphony_api.format_spot_quote(response)
    ...
    async def fetch_spot_ohlc(self):
        try:
            while True:
                current_datetime = datetime.now()
                ...
                end_time = (datetime.now().replace(hour=16, minute=00, second=0)).strftime("%b %d %Y %H%M%S")
                response = self.symphony_api.get_ohlc(exchangeSegment='NSECM', exchangeInstrumentID=26000, startTime=start_time, endTime=end_time, compressionValue=constants.compression_value_15_min)
...
class OptionsCandleEngine:
    ...
    async def fetch_options_ohlc(self, excahngeInstrumentID, start_time, end_time):
        try:
                response = self.symphony_api.get_ohlc(exchangeSegment='NSEFO', exchangeInstrumentID=excahngeInstrumentID, startTime=start_time, endTime=end_time, compressionValue=self.compression_value)
                return self.symphony_api.format_ohlc_options(response)
        except Exception as e:
            self.logger.log_message('error', f"Failed to fetch options ohlc for instrument {excahngeInstrumentID}: {e}")
    ...
    async def get_options_instruments(self, spot_close_price, exchangeSegment=['NSEFO']):
        try:
            # rounding off the close price to the nearest 50
            spot_close_price_rounded = round(spot_close_price/50)*50

            # make a list of 20 strike prices above and below the spot close price at interval of 50
            strike_prices = [spot_close_price_rounded + i*50 for i in range(-20, 20)]

            # get closest expiry date
            # exchangeSegment: int, series: str, symbol: str
            closest_expiry_date = self.symphony_api.get_closest_expiry_date(exchangeSegment=2, symbol='NIFTY', series='OPTIDX')
            
            df_formatted_masters = self.symphony_api.get_formatted_masters(exchangeSegment=exchangeSegment)
            ...
            filtered_df.to_csv('instruments.csv')
