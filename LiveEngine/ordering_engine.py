...
class SymphonyInteractiveAPI:
    def __init__(self):
        ...
        self.__token = None
        self.__multiplier = 0
        self.__base_quantity = 25
        self.__freeze_quantity = 1800
        self.__quantity = 0
        self.logger = CustomLogger(filename="SymphonyInteractiveAPI")
        self.trade_sheet = pd.DataFrame(columns=['TradingSymbol', 'InstrumentID', 'CMP', 'LegPosition', 'BuyAveragePrice', 'BuyQuantity', 'SellAveragePrice', 'SellQuantity', 'NetQuantity', 'UnrealisedM2M', 'RealisedM2M', 'M2M'])
        ...
        self.daily_json_path = constants.DAILY_JSON_PATH
    ...
    def update_headers(self, token):
        if token is None:
            ...
            # write the headers to the json file
    ...
    def read_trade_sheet(self):
        try:
            return pd.read_csv("LiveEngine/trade_sheet.csv")
        except Exception as e:
            self.logger.log_message("error", f"Error in read_trade_sheet: {e}")

    def write_trade_sheet(self, data):
        try:
            data.to_csv("LiveEngine/trade_sheet.csv", index=False)
        except Exception as e:
            self.logger.log_message("error", f"Error in write_trade_sheet: {e}")

    def reset_trade_sheet(self):
        try:
            data = pd.DataFrame(columns=['TradingSymbol', 'CMP', 'LegPosition', 'BuyAveragePrice', 'BuyQuantity', 'SellAveragePrice', 'SellQuantity', 'NetQuantity', 'UnrealisedM2M', 'RealisedM2M', 'M2M'])
    ...
    def calculate_m2m(self):
        try:
            ...
            def calculate_realised_m2m(row):
                if row['BuyQuantity'] != 0 and row['SellQuantity'] != 0:
    ...
    def check_stoploss(self, position):
        try:
            ...
            print(f"Current M2M: {current_m2m}, Stoploss at: {stoploss_price}")
    ...
    def place_order_handler(self, data, prev_position, hard_squareoff=False):
        ...
        if position == "beginx":
            ...
            quantity = self.__multiplier * self.__base_quantity
            self.tradesheet = self.read_trade_sheet()
                    ...
                    while orderQuantity > 0:
                        ...
                        if 'filled' in order_status.lower():
                            ...
                            if exchangeInstrumentID not in self.trade_sheet['InstrumentID'].values:
                                ...
                                new_row.at[0, 'TradingSymbol'] = trading_symbol
                            ...
                            condition = self.trade_sheet['TradingSymbol'] == trading_symbol

                            self.trade_sheet.loc[condition, f'{order_side.capitalize()}Quantity'] += quantity_response
                            
                            current_qty = self.trade_sheet.loc[condition, f'{order_side.capitalize()}Quantity']
                            current_avg_price = self.trade_sheet.loc[condition, f'{order_side.capitalize()}AveragePrice']
                            new_avg_price = ((current_avg_price * current_qty) + (order_price * quantity_response)) / (current_qty + quantity_response)
                            self.trade_sheet.loc[condition, f'{order_side.capitalize()}AveragePrice'] = new_avg_price
                            ...
                            response_codes[trading_symbol] = 200
                            ...
                            telegramBot.send_message(f"Order placed for\n{trading_symbol}\nTicker:{strike_prices[i]}\nOrderSide: {order_side}\nQuantity: {orderQuantity}\nOrderPrice: {order_price}")
                            ...
                            for j in range(3):
                                ...
                                if 'filled' in order_status.lower():
                                    # create a new row in the trade sheet if trading symbol is not present, it is not based on index
                                    if trading_symbol not in self.trade_sheet['TradingSymbol'].values:
                                        # Create a new row with default values for other columns and TradingSymbol as trading_symbol
                                        new_row = {col: 0 for col in self.trade_sheet.columns}
                                        new_row['TradingSymbol'] = trading_symbol
                                        self.trade_sheet = self.trade_sheet.append(new_row, ignore_index=True)
                                    ...
                                    self.logger.log_message("info", f"Order placed for {trading_symbol} leg {i} at retry level: {j}, Instrument: {exchangeInstrumentID}; OrderSide: {orderSide}; Quantity: {orderQuantity}\n{order_status}, {message}")
...
if __name__ == "__main__":
    try:
        symphony_api = SymphonyInteractiveAPI()
        symphony_api.login()
        response = symphony_api.place_order(
            exchangeSegment="NSEFO",
            exchangeInstrumentID=56922,
            productType="NRML",
            orderType="MARKET",
            orderSide="SELL",
            timeInForce="DAY",
            ...
            limitPrice=0,
            stopPrice=0,
            orderUniqueIdentifier="First;Test;Hirak-123",
            clientID='*****'
        )
        print("hewkndf")
        print(response.json())
        print("hewkndf")
        # get tradebook
        # response = symphony_api.get_tradebook()
        # print(response.json())
        ...
        # call app order id
        response = symphony_api.get_order_history(120009834, "*****")
        print(response.json())
