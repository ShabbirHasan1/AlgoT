        ...
        try:
            ...
            for i in range(1, len(self.df)):
                if self.df.at[self.df.index[i], 'PH'] == True:
                    ...
                    self.df.at[self.df.index[i], 'upper'] = int(self.df.at[self.df.index[i - 1], 'upper'] - self.df.at[self.df.index[i], 'slope_ph'])
            ...
            for i in range(1, len(self.df)):
                if self.df.at[self.df.index[i], 'PL'] == True:
                    ...
                    self.df.at[self.df.index[i], 'lower'] = int(self.df.at[self.df.index[i - 1], 'lower'] + self.df.at[self.df.index[i], 'slope_pl'])
...
class OptionsBrain:
    ...
    async def put_instruments(self):
        try:
            ...
            self.options_df = df
            # implement file locking while writing to csv
...
class StopLossBrain:
    ...
    async def populate_dataframe(self):
        self.options_df = pd.read_csv('options_brain.csv')
    ...
    # async def get_all_leg_prices(self):
    #     try:
    #         # Get current prices for all instruments in one go
    #         prices = {
    #             'legPriceOg1': await self.spot_candle_engine.fetch_ohlc_once(exchangeInstrumentID=int(self.options_df['atmce']), exchangeSegment='NSEFO', compressionValue=constants.compression_value_1_min, lookbackDays=0, drop_last=True, subtract_time=False),
    #             'legPriceOg2': await self.spot_candle_engine.fetch_ohlc_once(exchangeInstrumentID=int(self.options_df['atmpe']), exchangeSegment='NSEFO', compressionValue=constants.compression_value_1_min, lookbackDays=0, drop_last=True, subtract_time=False),
    #             'legPriceOg3': await self.spot_candle_engine.fetch_ohlc_once(exchangeInstrumentID=int(self.options_df['wce']), exchangeSegment='NSEFO', compressionValue=constants.compression_value_1_min, lookbackDays=0, drop_last=True, subtract_time=False),
    #             'legPriceOg4': await self.spot_candle_engine.fetch_ohlc_once(exchangeInstrumentID=int(self.options_df['wpe']), exchangeSegment='NSEFO', compressionValue=constants.compression_value_1_min, lookbackDays=0, drop_last=True, subtract_time=False)
    #         }
    #         # self.leg_prices = {key: value['Close'] for key, value in prices.items()}
    #         # print(self.leg_prices)

    #         # convert close for each leg to 15 min from 1 min
    #         for key, value in prices.items():
    #             self.leg_prices[key] = value.resample('15T').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last'}).dropna()
    ...
    async def write_to_csv(self):
        try:
            ...
            trade_sheet = pd.read_csv('LiveEngine/trade_sheet.csv')
            ...
            trade_sheet.to_csv('LiveEngine/trade_sheet.csv', index=False)
        except Exception as e:
